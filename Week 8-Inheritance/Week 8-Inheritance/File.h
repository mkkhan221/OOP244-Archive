...
namespace sdds {
	...
	public:
		...
		File(const char*, const char* fid = nullptr, int s = 0);
		...
		std::ostream& display(std::ostream&, const char* msg);
		const File& getFileName(char*)const;
		const File& getFileID(char*)const;
		int getSize()const;
